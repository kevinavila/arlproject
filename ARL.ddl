-- Generated by Oracle SQL Developer Data Modeler 4.0.3.853
--   at:        2015-04-20 22:34:53 CDT
--   site:      Oracle Database 11g
--   type:      Oracle Database 11g




DROP TABLE NL_Division CASCADE CONSTRAINTS ;

DROP TABLE NL_Document CASCADE CONSTRAINTS ;

DROP TABLE NL_Lab CASCADE CONSTRAINTS ;

DROP TABLE NL_Member CASCADE CONSTRAINTS ;

DROP TABLE NL_Membership CASCADE CONSTRAINTS ;

CREATE TABLE NL_Division
  (
    division_id   VARCHAR2 (4) NOT NULL ,
    NL_Lab_lab_id VARCHAR2 (4)
  ) ;
ALTER TABLE NL_Division ADD CONSTRAINT NL_Division_PK PRIMARY KEY ( division_id ) ;

CREATE TABLE NL_Document
  (
    document_id   INTEGER NOT NULL ,
    filename      VARCHAR2 (4000 BYTE) ,
    file_mimetype VARCHAR2 (512) ,
    file_charset  VARCHAR2 (512) ,
    file_data BLOB ,
    file_comments       VARCHAR2 (4000) ,
    tags                VARCHAR2 (4000) ,
    NL_Member_member_id INTEGER
  ) ;
ALTER TABLE NL_Document ADD CONSTRAINT NL_Document_PK PRIMARY KEY ( document_id ) ;

CREATE TABLE NL_Lab
  ( lab_id VARCHAR2 (4) NOT NULL
  ) ;
ALTER TABLE NL_Lab ADD CONSTRAINT NL_Lab_PK PRIMARY KEY ( lab_id ) ;

CREATE TABLE NL_Member
  (
    member_id             INTEGER NOT NULL ,
    type                  VARCHAR2 (255) ,
    name                  VARCHAR2 (255) ,
    status                VARCHAR2 (1) ,
    status_effective_date DATE ,
    division              VARCHAR2 (4) ,
    lab_code              VARCHAR2 (4) ,
    description           VARCHAR2 (4000) ,
    external_name         VARCHAR2 (255)
  ) ;
ALTER TABLE NL_Member ADD CONSTRAINT NL_Member_PK PRIMARY KEY ( member_id ) ;

CREATE TABLE NL_Membership
  (
    membership_id               INTEGER NOT NULL ,
    date_added                  DATE ,
    membership_division         VARCHAR2 (4) ,
    ARLNL_List_Member_member_id INTEGER ,
    NL_Member_member_id         INTEGER
  ) ;
ALTER TABLE NL_Membership ADD CONSTRAINT NL_Membership_PK PRIMARY KEY ( membership_id ) ;

ALTER TABLE NL_Division ADD CONSTRAINT NL_Division_NL_Lab_FK FOREIGN KEY ( NL_Lab_lab_id ) REFERENCES NL_Lab ( lab_id ) ;

ALTER TABLE NL_Document ADD CONSTRAINT NL_Document_NL_Member_FK FOREIGN KEY ( NL_Member_member_id ) REFERENCES NL_Member ( member_id ) ;

ALTER TABLE NL_Membership ADD CONSTRAINT NL_Membership_NL_Member_FK FOREIGN KEY ( ARLNL_List_Member_member_id ) REFERENCES NL_Member ( member_id ) ;

ALTER TABLE NL_Membership ADD CONSTRAINT NL_Membership_NL_Member_FKv1 FOREIGN KEY ( NL_Member_member_id ) REFERENCES NL_Member ( member_id ) ;


-- Oracle SQL Developer Data Modeler Summary Report: 
-- 
-- CREATE TABLE                             5
-- CREATE INDEX                             0
-- ALTER TABLE                              9
-- CREATE VIEW                              0
-- CREATE PACKAGE                           0
-- CREATE PACKAGE BODY                      0
-- CREATE PROCEDURE                         0
-- CREATE FUNCTION                          0
-- CREATE TRIGGER                           0
-- ALTER TRIGGER                            0
-- CREATE COLLECTION TYPE                   0
-- CREATE STRUCTURED TYPE                   0
-- CREATE STRUCTURED TYPE BODY              0
-- CREATE CLUSTER                           0
-- CREATE CONTEXT                           0
-- CREATE DATABASE                          0
-- CREATE DIMENSION                         0
-- CREATE DIRECTORY                         0
-- CREATE DISK GROUP                        0
-- CREATE ROLE                              0
-- CREATE ROLLBACK SEGMENT                  0
-- CREATE SEQUENCE                          0
-- CREATE MATERIALIZED VIEW                 0
-- CREATE SYNONYM                           0
-- CREATE TABLESPACE                        0
-- CREATE USER                              0
-- 
-- DROP TABLESPACE                          0
-- DROP DATABASE                            0
-- 
-- REDACTION POLICY                         0
-- 
-- ERRORS                                   0
-- WARNINGS                                 0
